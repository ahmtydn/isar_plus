...
description: Learn how to use Isar across multiple Dart isolates for better performance
icon: Cpu
---
...
<Callout type="info" title="When to Use Isolates">
...
<Callout type="warn" title="Schema Consistency">
...
Synchronous transactions (`write()`) are faster than async transactions (`writeAsync()`) and are ideal for background isolates where blocking is acceptable.
</Callout>
