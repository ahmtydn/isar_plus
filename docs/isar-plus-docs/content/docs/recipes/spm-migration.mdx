---
title: SPM Migration
description: Migrating isar_plus_flutter_libs from CocoaPods to Swift Package Manager with a unified Darwin shared structure
icon: Package
---
...
dependencies:
  ...
  isar_plus_flutter_libs: {versions.isar_plus_flutter_libs}
```
...
Delete the `Podfile` and `Podfile.lock` from your iOS project directory, and remove the `Pods/` directory:
