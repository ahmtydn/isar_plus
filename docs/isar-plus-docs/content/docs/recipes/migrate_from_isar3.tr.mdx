...
Burada durup derleme hatalarını giderin (örneğin, `Id?` alanlarının non-null `int id` olması veya `Isar.autoIncrement` kullanması gerekir).
...
@collection
class User {
  int id = Isar.autoIncrement; // ya da @id
}
```
</Tab>
...
v4 ID'lerinin benzersiz, null olmayan tamsayılar olması gerektiğini unutmayın. Kopyalama sırasında `Isar.autoIncrement` kullanın veya deterministik anahtarlar üretin.
</Callout>
