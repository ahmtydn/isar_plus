...
await isar.writeAsync((isar) async {
  final user2 = User()
    ..username = 'john_doe'
    ..age = 30;
  isar.users.put(user2); // ❌ Error: unique constraint violated
});
```
...
await isar.writeAsync((isar) async {
  ...
  isar.users.putByUsername(user);

  // The existing id is reused
  final user2 = User()
    ..username = 'john_doe'
    ..age = 30;
  isar.users.putByUsername(user2);
